...
class SimpleSAML_SessionHandlerPHP extends SimpleSAML_SessionHandler
{
	...
	// SWARMING WORKAROUND: Reset  session if we have an old session that was an error during the migration to AWS
	if (strpos($session, 'No PATH_INFO to module.php') !== false || strpos($session, 'www.brainmdhealth.com:80') !== false ) {
		return null;
	}
